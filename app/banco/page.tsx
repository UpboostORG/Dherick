...
export default function BancoEmergencia() {
  const [eur, setEur] = useState(775);
  const [usd, setUsd] = useState(812);
  ...
  useEffect(() => {
    const e = localStorage.getItem("__banco_eur_v5");
    const u = localStorage.getItem("__banco_usd_v5");
  ...
  useEffect(() => {
    if (!loaded) return;
    localStorage.setItem("__banco_eur_v5", String(eur));
    localStorage.setItem("__banco_usd_v5", String(usd));
