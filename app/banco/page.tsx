...
export default function BancoEmergencia() {
  const [eur, setEur] = useState(858);
  const [usd, setUsd] = useState(856);
  ...
  useEffect(() => {
    const e = localStorage.getItem("__banco_eur_v6");
    const u = localStorage.getItem("__banco_usd_v6");
  ...
  useEffect(() => {
    if (!loaded) return;
    localStorage.setItem("__banco_eur_v6", String(eur));
    localStorage.setItem("__banco_usd_v6", String(usd));
